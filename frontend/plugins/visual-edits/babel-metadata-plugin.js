...
const babelMetadataPlugin = ({ types: t }) => {
   ...
   */
  function analyzeExpression(exprPath, state, options = {}) {
    ...
    if (t.isIdentifier(node)) {
      return analyzeIdentifier(node.name, exprPath, state, options);
    }

    // Handle MemberExpression (like item.name or obj.prop.value)
    if (t.isMemberExpression(node)) {
      return analyzeMemberExpression(exprPath, state, options);
    }
  ...
  function analyzeIdentifier(name, exprPath, state, options = {}) {
    const { skipArrayContext = false, visited = new Set(), depth = 0 } = options;

    if (depth > 10) return { type: "unknown", varName: name, isEditable: false };
    const nextOptions = { skipArrayContext, visited, depth: depth + 1 };
    ...
    if (!skipArrayContext) {
      const arrayContext = getArrayIterationContext(exprPath, state, nextOptions);
    ...
    if (bindingPath.isIdentifier() && bindingPath.parentPath.isFunctionDeclaration()) {
      ...
      const tracedSource = tracePropToSource(name, componentName, exprPath, state, nextOptions);
   ...
   */
  function tracePropToSource(propName, componentName, exprPath, state, options = {}) {
          ...
          if (attrPath) {
            ...
            if (valuePath && valuePath.isJSXExpressionContainer()) {
              ...
              if (innerExpr && innerExpr.node) {
                tracedSource = analyzeExpression(innerExpr, state, options);
              }
            }
          }
        }
    ...
    for (const [absPath, cached] of FILE_AST_CACHE) {
      ...
      traverse(ast, {
        ImportDeclaration(importPath) {
          ...
          const parentPath = importPath.parentPath?.parentPath;
   ...
   */
  function getArrayIterationContext(exprPath, state, options = {}) {
    ...
    if (t.isIdentifier(arrayNode)) {
      ...
      const arrayInfo = analyzeIdentifier(arrayVar, callExprParent.get("callee.object"), state, { ...options, skipArrayContext: true });
